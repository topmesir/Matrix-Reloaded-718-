...
public final class Utilities {
	...
	public static void copyFile(File sourceFile, File destFile) throws IOException {
		...
		try (FileInputStream fos = new FileInputStream(sourceFile)) {
			try(FileOutputStream destination = new FileOutputStream(destFile)) {
				destination.getChannel().transferFrom(fos.getChannel(), 0, fos.getChannel().size());
			}
		}
	}
	...
	public static Class<?>[] getAllClasses(String directory) throws IOException, ClassNotFoundException {
		String path = "bin/" + directory.replace('.', '/') + "/";
		File dir = new File(path);
		List<Class<?>> classes = new ArrayList<>();
		List<File> files = (List<File>) FileUtils.listFiles(dir, TrueFileFilter.INSTANCE, TrueFileFilter.INSTANCE);
		for (File file : files) {
			if (file.getName().endsWith(".class")) {
				classes.add(Class.forName(file.getParent().replace("\\", ".").replace("bin.", "") + '.'
	...
	public static byte[] completeQuickMessage(Player player, int fileId, byte[] data) {
		if (fileId == 1) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.AGILITY) };
		} else if (fileId == 8) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.ATTACK) };
		} else if (fileId == 13) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.CONSTRUCTION) };
		} else if (fileId == 16) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.COOKING) };
		} else if (fileId == 23) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.CRAFTING) };
		} else if (fileId == 30) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.DEFENCE) };
		} else if (fileId == 34) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.FARMING) };
		} else if (fileId == 41) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.FIREMAKING) };
		} else if (fileId == 47) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.FISHING) };
		} else if (fileId == 55) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.FLETCHING) };
		} else if (fileId == 62) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.HERBLORE) };
		} else if (fileId == 70) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.HITPOINTS) };
		} else if (fileId == 74) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.HUNTER) };
		} else if (fileId == 135) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.MAGIC) };
		} else if (fileId == 127) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.MINING) };
		} else if (fileId == 120) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.PRAYER) };
		} else if (fileId == 116) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.RANGE) };
		} else if (fileId == 111) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.RUNECRAFTING) };
		} else if (fileId == 103) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.SLAYER) };
		} else if (fileId == 96) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.SMITHING) };
		} else if (fileId == 92) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.STRENGTH) };
		} else if (fileId == 85) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.SUMMONING) };
		} else if (fileId == 79) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.THIEVING) };
		} else if (fileId == 142) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.WOODCUTTING) };
		} else if (fileId == 990) {
			data = new byte[] { (byte) player.getSkills().getLevelForXp(SkillType.DUNGEONEERING) };
		} else if (fileId == 965) {
