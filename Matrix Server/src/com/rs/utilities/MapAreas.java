...
public final class MapAreas {
	...
	public static final boolean isAtArea(String areaName, WorldTile tile) {
		return isAtArea(Utilities.getNameHash(areaName), tile);
	}
	...
	public static final int getRandomAreaHash() {
		synchronized (lock) {
			while (true) {
				long id = Utilities.getRandom(Integer.MAX_VALUE) + Utilities.getRandom(Integer.MAX_VALUE);
	...
	private static void loadUnpackedMapAreas() {
		...
		try {
			try(BufferedReader in = new BufferedReader(new FileReader("data/map/unpackedMapAreas.txt"))) {
				try(DataOutputStream out = new DataOutputStream(new FileOutputStream(PACKED_PATH))) {
					String line;
					while ((line = in.readLine()) != null) {
						...
						int areaNameHash = Utilities.getNameHash(areaName);
