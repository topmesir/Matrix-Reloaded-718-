...
import com.rs.game.entity.mobile.player.content.type.hint.HintIcon;
...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendProjectile(MobileEntity receiver, WorldTile startTile, WorldTile endTile, int gfxId, int startHeight, int endHeight, int speed, int delay, int curve, int startDistanceOffset, int creatorSize) {
		...
		int duration = (Utilities.getDistance(startTile.getX(), startTile.getY(), endTile.getX(), endTile.getY()) * 30 / ((speed / 10) < 1 ? 1 : (speed / 10))) + delay;
	...
	public void sendFriendsChatChannel() {
		FriendsChatManager manager = player.getCurrentFriendChat();
	...
	public void sendFriends() {
		...
		for (String username : player.getContent().get(ContentType.FRIENDSLIST).getFriends()) {
			String displayName;
			...
			player.getPackets().sendFriend(Utilities.formatPlayerNameForDisplay(username), displayName, 1, p2 != null && player.getContent().get(ContentType.FRIENDSLIST).isOnline(p2), false, stream);
		}
		stream.endPacketVarShort();
		session.write(stream);
	}
	...
	public void sendFriend(String username, String displayName, int world, boolean putOnline, boolean warnMessage, OutputStream stream) {
		...
		stream.writeByte(player.getContent().get(ContentType.FRIENDSLIST).getRank(Utilities.formatPlayerNameForProtocol(username)));
		stream.writeByte(0);
		if (putOnline) {
	...
	public void sendIgnores() {
		...
		stream.writeByte(player.getContent().get(ContentType.FRIENDSLIST).getIgnores().size());
		for (String username : player.getContent().get(ContentType.FRIENDSLIST).getIgnores()) {
			String display;
			Player p2 = World.getPlayerByDisplayName(username);
			if (p2 != null)
	...
	public void sendGameBarStages() {
		...
		sendConfig(2159, player.getContent().get(ContentType.FRIENDSLIST).getFriendsChatStatus());
	...
	public void sendPrivateGameBarStage() {
		...
		stream.writeByte(player.getContent().get(ContentType.FRIENDSLIST).getPrivateStatus());
		session.write(stream);
	}
	...
	public void sendCutscene(int id) {
		...
		for (int count = 0; count < 20; count++)
			...
			for (int i = 0; i < 4; i++)
				stream.writeInt(0);
		byte[] appearence = player.getContent().get(ContentType.APPEARANCE).getAppeareanceData();
	...
	public void sendMessage(int type, String text, Player p) {
		...
		if ((maskData & 0x1) != 0) {
			...
			if (p.hasDisplayName())
				stream.writeString(Utilities.formatPlayerNameForDisplay(p.getUsername()));
		}
		stream.writeString(text);
		stream.endPacketVarByte();
		session.write(stream);
	...
	public void sendSkillLevel(SkillType skill) {
		...
		stream.write128Byte(skill.getSkillId());
