...
public final class ClientScriptMapDefinitions {
	...
	private static final ConcurrentHashMap<Integer, ClientScriptMapDefinitions> interfaceScripts = new ConcurrentHashMap<Integer, ClientScriptMapDefinitions>();
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 10000; i++) {
			if (ClientScriptMapDefinitions.getMap(i).getSize() == 32)
	...
	public static final ClientScriptMapDefinitions getMap(int scriptId) {
		ClientScriptMapDefinitions script = interfaceScripts.get(scriptId);
		if (script != null)
			return script;
		byte[] data = Cache.STORE.getIndexes()[17].getFile(scriptId >>> 0xba9ed5a8, scriptId & 0xff);
		script = new ClientScriptMapDefinitions();
		if (data != null)
	...
	private void readValues(InputStream stream, int opcode) {
		if (opcode == 1)
			aChar6337 = Utilities.method2782((byte) stream.readByte());
		else if (opcode == 2)
			aChar6345 = Utilities.method2782((byte) stream.readByte());
		else if (opcode == 3)
	...
	private ClientScriptMapDefinitions() {
