...
public class Region {
	...
	private static final int getMusicId(String musicName) {
		...
		if (musicName.equals(""))
			return -2;
		int musicIndex = (int) ClientScriptMapDefinitions.getMap(1345).getKeyForValue(musicName);
		return ClientScriptMapDefinitions.getMap(1351).getIntValue(musicIndex);
	}
			...
			while ((incr = landStream.readSmart2()) != 0) {
				...
				while ((incr2 = landStream.readUnsignedSmart()) != 0) {
					...
					if (objectPlane < 0 || objectPlane >= 4 || plane < 0 || plane >= 4)
						continue;
					addObject(new GameObject(objectId, type, rotation, localX + regionX, localY + regionY, objectPlane), objectPlane, localX, localY);
				}
			}
		}
	...
	public GameObject getObject(int id, WorldTile tile) {
	...
	public GameObject getRemovedObject(WorldTile tile) {
