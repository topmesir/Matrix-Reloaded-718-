...
public final class LocalPlayerUpdate {
	...
	private void processOutsidePlayers(OutputStream stream, OutputStream updateBlockData, boolean nsn2) {
		...
		for (int i = 0; i < outPlayersIndexesCount; i++) {
			...
			if (needsAdd(p)) {
				...
				boolean needAppearenceUpdate = needAppearenceUpdate(p.getIndex(), p.getContent().get(ContentType.APPEARANCE).getMD5AppeareanceDataHash());
			...
			if (needsRemove(p)) {
				...
				if (p.hasTeleported()) {
					...
					for (int i2 = i + 1; i2 < localPlayersIndexesCount; i2++) {
						...
						if (needsRemove(p2) || p2.hasTeleported() || p2.getNextWalkDirection() != -1 || (p2.needMasksUpdate() || needAppearenceUpdate(p2.getIndex(), p2.getContent().get(ContentType.APPEARANCE).getMD5AppeareanceDataHash())))
							break;
						skip++;
					}
					skipPlayers(stream, skip);
	...
	private void applyAppearanceMask(Player p, OutputStream data) {
		byte[] renderData = p.getContent().get(ContentType.APPEARANCE).getAppeareanceData();
		totalRenderDataSentLength += renderData.length;
		cachedAppearencesHashes[p.getIndex()] = p.getContent().get(ContentType.APPEARANCE).getMD5AppeareanceDataHash();
