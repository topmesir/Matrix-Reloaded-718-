...
public final class Inventory extends AbstractContent {
	...
	public boolean addItem(int itemId, int amount) {
		if (itemId < 0 || amount < 0 || !Utilities.itemExists(itemId) || !player.getControlerManager().canAddInventoryItem(itemId, amount))
			return false;
	...
	public boolean addItem(Item item) {
		if (item.getId() < 0 || item.getAmount() < 0 || !Utilities.itemExists(item.getId()) || !player.getControlerManager().canAddInventoryItem(item.getId(), item.getAmount()))
			return false;
		Item[] itemsBefore = items.getItemsCopy();
		if (!items.add(item)) {
