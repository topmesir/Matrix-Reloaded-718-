package com.rs.game.entity.mobile.player.content.type.action.impl;
...
import com.rs.game.entity.mobile.player.content.type.action.Action;
...
public class PlayerCombat extends Action {
	...
	public PlayerCombat(MobileEntity target) {
		this.target = target;
	}
	...
	public int processWithDelay(Player player) {
		...
		int spellId = player.getContent().get(ContentType.COMBATDEF).getSpellId();
		...
		if (spellId > 0) {
			...
			Item gloves = player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_HANDS);
			spellcasterGloves = gloves != null && gloves.getDefinitions().getName().contains("Spellcaster glove") && player.getContent().get(ContentType.EQUIPMENT).getWeaponId() == -1 && new Random().nextInt(30) == 0 ? spellId : -1;
	...
	private void addAttackedByDelay(MobileEntity player) {
		target.setAttackedBy(player);
		target.setAttackedByDelay(Utilities.currentTimeMillis() + 8000); // 8seconds
	}
	...
	public MobileEntity[] getMultiAttackTargets(Player player) {
		return getMultiAttackTargets(player, 1, 9);
	}

	public MobileEntity[] getMultiAttackTargets(Player player, int maxDistance, int maxAmtTargets) {
		List<MobileEntity> possibleTargets = new ArrayList<MobileEntity>();
			...
			y: for (int regionId : target.getMapRegionsIds()) {
				...
				if (target instanceof Player) {
					...
					for (int npcIndex : npcIndexes) {
						...
						if (n == null || n == target || n.isDead() || n.hasFinished() || !n.isAtMultiArea() || !n.withinDistance(target, maxDistance) || !n.getDefinitions().hasAttackOption() || !player.getControlerManager().canHit(n))
							continue;
		...
		return possibleTargets.toArray(new MobileEntity[possibleTargets.size()]);
	}

	public int mageAttack(final Player player, int spellId, boolean autocast) {
		if (!autocast) {
			player.getContent().get(ContentType.COMBATDEF).resetSpells(false);
			player.getContent().get(ContentType.ACTION).forceStop();
		}
		if (!Magic.checkCombatSpell(player, spellId, -1, true)) {
			if (autocast)
				player.getContent().get(ContentType.COMBATDEF).resetSpells(true);
			return -1; // stops
		}
		if (spellId == 65535) {
			player.setNextFaceEntity(target);
			...
			mage_hit_gfx = 2036;
			delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, (5 * player.getSkills().getLevel(SkillType.MAGIC)) - 180)));
			World.sendProjectile(player, target, 2035, 60, 32, 50, 50, 0, 0);
			return 4;
		}
		if (player.getContent().get(ContentType.COMBATDEF).getSpellBook() == 192) {
			switch (spellId) {
			case 25: // air strike
				...
				base_mage_xp = 5.5;
				int baseDamage = 20;
				if (player.getContent().get(ContentType.EQUIPMENT).getGlovesId() == 205) {
					baseDamage = 90;
				}
			...
			case 55:// snare
				...
				if (snareHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Utilities.currentTimeMillis())
			...
			case 81:// entangle
				...
				if (entangleHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Utilities.currentTimeMillis())
			...
			case 34: // air bolt
				...
				base_mage_xp = 13.5;
				baseDamage = 90;
				if (player.getContent().get(ContentType.EQUIPMENT).getGlovesId() == 777) {
					baseDamage = 120;
				}
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 2699, 18, 18, 50, 50, 0, 0);
			...
			case 80: // fire wave
				...
				return 5;
			case 86: // teleblock
				if (target instanceof Player && ((Player) target).getTeleBlockDelay() <= Utilities.currentTimeMillis()) {
			...
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				mage_hit_gfx = 1019;
				base_mage_xp = 70;
				int boost = (player.getSkills().getLevelForXp(SkillType.MAGIC) - 77) * 5;
				...
				return player.getContent().get(ContentType.EQUIPMENT).getWeaponId() == 21777 ? 4 : 5;
			}
		} else if (player.getContent().get(ContentType.COMBATDEF).getSpellBook() == 193) {
			switch (spellId) {
			case 28:// Smoke Rush
			...
			case 23: // ice barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						delayMagicHit(Utilities.getDistance(player, target) > 3 ? 4 : 2, hit);
						World.sendProjectile(player, target, 368, 60, 32, 50, 50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
								return false;
	...
	public void attackTarget(MobileEntity[] targets, MultiAttack perform) {
		MobileEntity realTarget = target;
		for (MobileEntity t : targets) {
			target = t;
			if (!perform.attack())
				break;
		}
	...
	public int getRandomMagicMaxHit(Player player, int baseDamage) {
		...
		if (current <= 0) // Splash.
			return -1;
		int hit = Utilities.random(current + 1);
		if (hit > 0) {
			if (target instanceof NPC) {
				NPC n = (NPC) target;
	 ...
	 * (Math.round((player.getSkills().getLevel(SkillType.MAGIC) *
	 * player.getContent().get(ContentKind.PRAYER).getMageMultiplier())) + 8) +
	 ...
	 * 1.3; effectiveAttack *=
	 * player.getContent().get(AbstractContentType.AURAS).getMagicAccurayMultiplier(); double totalDefence
	 * = 0; if (target instanceof Player) { Player p2 = (Player) target; double
	 * effectiveDefence = (Math.round(p2.getSkills().getLevel(SkillType.DEFENCE) +
	 * (p2.getContent().get(ContentKind.PRAYER).getDefenceMultiplier() + Utils.random(0, 14))) + 8) +
	 * p2.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_DEF];
	 * double effectiveMagicDefence = p2.getSkills().getLevel(SkillType.MAGIC) +
	 * p2.getContent().get(ContentKind.PRAYER).getMageMultiplier(); effectiveMagicDefence *= 0.7;
	 ...
	 * player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_DEF]))
	 ...
	 * ((player.getSkills().getLevel(SkillType.MAGIC) - player
	 * .getSkills().getLevelForXp(SkillType.MAGIC)) * 0.03); if (boost > 1) max_hit
	 * *= boost; double magicPerc =
	 * player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions
	 ...
	 * (Math.round(player.getSkills().getLevel(SkillType.MAGIC) *
	 * player.getContent().get(ContentKind.PRAYER).getMageMultiplier()) + 8); if
	 * (fullVoidEquipped(player, new int[] { 11663, 11674 })) EA *= 1.3; EA *=
	 * player.getContent().get(AbstractContentType.AURAS).getMagicAccurayMultiplier(); double ED = 0, DB;
	 * if (target instanceof Player) { Player p2 = (Player) target; double EMD =
	 * p2.getSkills().getLevel(SkillType.MAGIC) *
	 * p2.getContent().get(ContentKind.PRAYER).getMageMultiplier(); if (p2.getFamiliar() != null &&
	 * (p2.getFamiliar().getId() == 6870 || p2.getFamiliar().getId() == 6871))
	 * EMD *= 1.05; Math.round(EMD); double ERD =
	 * (Math.round(p2.getSkills().getLevel(SkillType.DEFENCE) *
	 * p2.getContent().get(ContentKind.PRAYER).getDefenceMultiplier()) + 8) +
	 * (player.getContent().get(ContentType.COMBATDEF).isDefensiveCasting() ? 3 : 0); EMD *= 0.7;
	 ...
	 * player
	 * .getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_ATTACK])) /
	...
	private int getMagicMaxHit(Player player, int baseDamage) {
		double EA = (Math.round(player.getSkills().getLevel(SkillType.MAGIC) * player.getContent().get(ContentType.PRAYER).getMageMultiplier()) + 11);
		if (fullVoidEquipped(player, new int[] { 11663, 11674 }))
			EA *= 1.3;
		EA *= player.getContent().get(ContentType.AURAS).getMagicAccurayMultiplier();
		double ED = 0, DB;
		if (target instanceof Player) {
			Player p2 = (Player) target;
			double EMD = ((Math.round(p2.getSkills().getLevel(SkillType.MAGIC) * p2.getContent().get(ContentType.PRAYER).getMageMultiplier()) + 8) + (player.getContent().get(ContentType.COMBATDEF).isDefensiveCasting() ? 3 : 0));
			Math.round(EMD);
			double ERD = (Math.round(p2.getSkills().getLevel(SkillType.DEFENCE) * p2.getContent().get(ContentType.PRAYER).getDefenceMultiplier()) + 8);
			EMD *= 0.7;
			ERD *= 0.3;
			ED = (EMD + ERD);
			DB = p2.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_DEF];
		} else {
			NPC n = (NPC) target;
			DB = ED = n.getBonuses() != null ? n.getBonuses()[CombatDefinitions.MAGIC_DEF] / 1.6 : 0;
		}
		double A = (EA * (1 + player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_ATTACK]) / 64);
		double D = (ED * (1 + DB)) / 64;
		...
		max_hit = baseDamage;
		double boost = 1 + ((player.getSkills().getLevel(SkillType.MAGIC) - player.getSkills().getLevelForXp(SkillType.MAGIC)) * 0.03);
		if (boost > 1)
			max_hit *= boost;
		double magicPerc = player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_DAMAGE];
	...
	private double getMagicAccuracy(Player e, int baseDamage, int extraDamage) {
		int magicLevel = e.getSkills().getLevel(SkillType.MAGIC) + 1;
		int magicBonus = e.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_ATTACK];
		if ((baseDamage << 2) < 60 && spellcasterGloves > 0 && spellcasterGloves != 28 && spellcasterGloves != 25) {
			magicBonus += 20;
		}
		double prayer = 1.0 * e.getContent().get(ContentType.PRAYER).getMageMultiplier();
	 ...
	 * type = CombatDefinitions.getXpStyle(p.getContent().get(ContentType.EQUIPMENT).getWeaponId(),
	 * p.getContent().get(ContentType.COMBATDEF).getAttackStyle()); style += type ==
	 * SkillType.DEFENCE ? 3 : type == CombatDefinitions.SHARED ? 1 : 0; } double
	 * defLvl = (p != null ? p.getSkills().getLevel(SkillType.DEFENCE) : ((NPC)
	 * e).getCombatLevel() * 0.7) * 0.3; defLvl += (p != null ?
	 * p.getSkills().getLevel(SkillType.MAGIC) : ((NPC) e).getCombatLevel() * 0.7)
	 * * 0.7; int defBonus = p != null ?
	 * p.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.MAGIC_DEF] :
	 * ((NPC) e).getBonuses() != null ? ((NPC)
	 * e).getBonuses()[CombatDefinitions.MAGIC_DEF] : 5; double defMult = 1.0;
	 * if (p != null) { defMult += p.getContent().get(ContentKind.PRAYER).getDefenceMultiplier(); }
	 * double defence = ((defLvl + (defBonus << 2)) + style) * defMult; return
	 * defence < 1 ? 1 : defence; }
	 */

	private int rangeAttack(final Player player) {
		final int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
		final int attackStyle = player.getContent().get(ContentType.COMBATDEF).getAttackStyle();
		int combatDelay = getRangeCombatDelay(weaponId, attackStyle);
		int soundId = getSoundId(weaponId, attackStyle);
		if (player.getContent().get(ContentType.COMBATDEF).isUsingSpecialAttack()) {
			...
			if (player.getContent().get(ContentType.COMBATDEF).hasRingOfVigour())
				specAmt *= 0.9;
			if (player.getContent().get(ContentType.COMBATDEF).getSpecialAttackPercentage() < specAmt) {
				player.getPackets().sendGameMessage("You don't have enough power left.");
				player.getContent().get(ContentType.COMBATDEF).desecreaseSpecialAttack(0);
				return combatDelay;
			}
			player.getContent().get(ContentType.COMBATDEF).desecreaseSpecialAttack(specAmt);
			switch (weaponId) {
			...
			case 15703:
			case 15704:
				int ammoId = player.getContent().get(ContentType.EQUIPMENT).getAmmoId();
			...
			case 14684: // zanik cbow
				...
				delayHit(2, weaponId, attackStyle, getRangeHit(player, getRandomMaxHit(player, weaponId, attackStyle, true, true, 1.0, true) + 30 + Utilities.getRandom(120)));
				dropAmmo(player);
				break;
			...
			case 13882:
				...
				if (hit > 0) {
					final MobileEntity finalTarget = target;
					WorldTasksManager.schedule(new WorldTask() {
						int damage = hit;
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					int damage = 0;
					int ammoId = player.getContent().get(ContentType.EQUIPMENT).getAmmoId();
					if (ammoId != -1 && Utilities.getRandom(10) == 5) {
						switch (ammoId) {
						case 9237:
						...
						case 9243:
							...
							break;
						case 9244:
							damage = getRandomMaxHit(player, weaponId, attackStyle, true, false, !CombatDefinitions.hasAntiFireShield(target) ? 1.45 : 1.0, true);
					...
					if (weaponId != 4740)
						...
						player.getContent().get(ContentType.EQUIPMENT).removeAmmo(ammoId, 1);
				} else if (weaponId == 15241) {// handcannon
					if (Utilities.getRandom(player.getSkills().getLevel(SkillType.FIREMAKING) << 1) == 0) {
						// explode
						player.setNextGraphics(new Graphics(2140));
						player.getContent().get(ContentType.EQUIPMENT).getItems().set(3, null);
						player.getContent().get(ContentType.EQUIPMENT).refresh(3);
						player.getContent().get(ContentType.APPEARANCE).generateAppearenceData();
						player.applyHit(new Hit(player, Utilities.getRandom(150) + 10, HitLook.REGULAR_DAMAGE));
					...
					if (target instanceof Player) {
						Player p = (Player) target;
						Item weapon = p.getContent().get(ContentType.EQUIPMENT).getItem(3);
						boolean slashBased = weapon != null;
						if (weapon != null) {
							int slash = p.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.SLASH_ATTACK];
							for (int i = 0; i < 5; i++) {
								if (p.getContent().get(ContentType.COMBATDEF).getBonuses()[i] > slash) {
									slashBased = false;
									break;
								}
							}
						}
						if (p.getContent().get(ContentType.INVENTORY).containsItem(946, 1) || slashBased) {
							delay /= 2;
						}
						if (p.getContent().get(ContentType.PRAYER).usingPrayer(0, 18) || p.getContent().get(ContentType.PRAYER).usingPrayer(1, 8)) {
							delay /= 2;
						}
						if (delay < 5000) {
							delay = 5000;
					...
					final int ammoId = player.getContent().get(ContentType.EQUIPMENT).getAmmoId();
					player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, ammoId), 0, 100));
	...
	public void dropAmmo(Player player, int quantity) {
		if (quantity == -2) {
			...
			final int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
			if (weaponId != -1) {
				if ((quantity == -3 && Utilities.getRandom(10) < 2) || (quantity != -3 && Utilities.getRandom(3) > 0)) {
					...
					player.getContent().get(ContentType.EQUIPMENT).removeAmmo(weaponId, quantity);
					return;
				}
				player.getContent().get(ContentType.EQUIPMENT).removeAmmo(weaponId, quantity);
			...
			if (Utilities.getRandom(3) > 0) {
				...
				player.getContent().get(ContentType.EQUIPMENT).removeAmmo(ammoId, quantity);
				return;
			}
			if (ammoId != -1) {
				player.getContent().get(ContentType.EQUIPMENT).removeAmmo(ammoId, quantity);
	...
	private int getRangeHitDelay(Player player) {
		return Utilities.getDistance(player.getX(), player.getY(), target.getX(), target.getY()) >= 5 ? 2 : 1;
	}

	private int meleeAttack(final Player player) {
		int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
		int attackStyle = player.getContent().get(ContentType.COMBATDEF).getAttackStyle();
		...
		if (player.getContent().get(ContentType.COMBATDEF).isUsingSpecialAttack()) {
			...
			case 23690:
				...
				delayNormalHit(weaponId, attackStyle, getMeleeHit(player, 50 + Utilities.getRandom(100)), getMeleeHit(player, getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.1, true)));
				soundId = 3853;
				break;
			case 1249:// d spear
			case 1263:
			...
			case 23681:
				...
				player.getContent().get(ContentType.PRAYER).restorePrayer((sgsdamage / 4) * 10);
			...
			case 23680:
				...
				if (target instanceof Player) {
					...
					int amountLeft;
					if ((amountLeft = targetPlayer.getSkills().drainLevel(SkillType.DEFENCE, damage / 10)) > 0) {
						if ((amountLeft = targetPlayer.getSkills().drainLevel(SkillType.STRENGTH, amountLeft)) > 0) {
							if ((amountLeft = targetPlayer.getSkills().drainLevel(SkillType.PRAYER, amountLeft)) > 0) {
								if ((amountLeft = targetPlayer.getSkills().drainLevel(SkillType.ATTACK, amountLeft)) > 0) {
									if ((amountLeft = targetPlayer.getSkills().drainLevel(SkillType.MAGIC, amountLeft)) > 0) {
										if (targetPlayer.getSkills().drainLevel(SkillType.RANGE, amountLeft) > 0) {
											break;
										}
									}
								}
					...
					if (hit > 0) {
						...
						if (hit > 0) {
							...
							if (hit > 0) {
								...
								hits = new int[] { 0, 0, 0, Utilities.getRandom(7) };
							}
						}
					}
				}
	...
	public void playSound(int soundId, Player player, MobileEntity target) {
		if (soundId == -1)
			return;
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			double att = player.getSkills().getLevel(ranging ? 4 : 0) + player.getContent().get(ContentType.COMBATDEF).getBonuses()[ranging ? 4 : CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)];
			if (weaponId == -2) {
				att += 82;
			}
			att *= ranging ? player.getContent().get(ContentType.PRAYER).getRangeMultiplier() : player.getContent().get(ContentType.PRAYER).getAttackMultiplier();
			if (fullVoidEquipped(player, ranging ? (new int[] { 11664, 11675 }) : (new int[] { 11665, 11676 })))
				att *= 1.1;
			if (ranging)
				att *= player.getContent().get(ContentType.AURAS).getRangeAccurayMultiplier();
			double def = 0;
			if (target instanceof Player) {
				Player p2 = (Player) target;
				def = (double) p2.getSkills().getLevel(SkillType.DEFENCE) + (2 * p2.getContent().get(ContentType.COMBATDEF).getBonuses()[ranging ? 9 : CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))]);
		...
		int hit = Utilities.random(max_hit + 1);
		if (target instanceof NPC) {
			NPC n = (NPC) target;
			if (n.getId() == 9463 && hasFireCape(player))
				hit += 40;
		}
		if (player.getContent().get(ContentType.AURAS).usingEquilibrium()) {
	...
	private final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double specMultiplier) {
		if (!ranging) {
			 ...
			 * player.getSkills().getLevel(SkillType.STRENGTH); int strBonus =
			 * player
			 * .getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.STRENGTH_BONUS
			 * ]; double strMult = player.getContent().get(ContentKind.PRAYER).getStrengthMultiplier();
			 * double xpStyle = CombatDefinitions.getXpStyle(weaponId,
			 * attackStyle); double style = xpStyle == SkillType.STRENGTH ? 3 :
			...
			double strengthLvl = player.getSkills().getLevel(SkillType.STRENGTH);
			SkillType xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
			double styleBonus = xpStyle == SkillType.STRENGTH ? 3 : xpStyle == SkillType.SHARED ? 1 : 0;
			double otherBonus = 1;
			...
			double effectiveStrength = 8 + Math.floor((strengthLvl * player.getContent().get(ContentType.PRAYER).getStrengthMultiplier()) + styleBonus);
			if (fullVoidEquipped(player, 11665, 11676))
				effectiveStrength = Math.floor(effectiveStrength * 1.1);
			double strengthBonus = player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.STRENGTH_BONUS];
			if (weaponId == -2) {
				strengthBonus += 82;
			}
			...
			double rangedLvl = player.getSkills().getLevel(SkillType.RANGE);
			double styleBonus = attackStyle == 0 ? 3 : attackStyle == 1 ? 0 : 1;
			double effectiveStrenght = Math.floor(rangedLvl * player.getContent().get(ContentType.PRAYER).getRangeMultiplier()) + styleBonus;

			if (fullVoidEquipped(player, 11664, 11675))
				effectiveStrenght += Math.floor((player.getSkills().getLevelForXp(SkillType.RANGE) / 5) + 1.6);
			double strengthBonus = player.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.RANGED_STR_BONUS];
	 ...
	 * (player.getContent().get(AbstractContentType.AURAS).usingEquilibrium()) { int perc25MaxHit = (int)
	 ...
	 * int attLvl = e.getSkills().getLevel(SkillType.ATTACK); int attBonus =
	 ...
	 * 1.0 * e.getContent().get(ContentKind.PRAYER).getAttackMultiplier(); double accuracyMultiplier =
	 ...
	 * weaponId) { int strLvl = e.getSkills().getLevel(SkillType.STRENGTH); int
	 * strBonus =
	 * e.getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.STRENGTH_BONUS];
	 * if (weaponId == -2) { strBonus += 82; } double strMult =
	 * e.getContent().get(ContentKind.PRAYER).getStrengthMultiplier(); double xpStyle =
	 * CombatDefinitions.getXpStyle(weaponId, attackStyle); double style =
	 * xpStyle == SkillType.STRENGTH ? 3 : xpStyle == CombatDefinitions.SHARED ? 1
	 ...
	 * ((Player) e).getContent().get(ContentType.COMBATDEF).getAttackStyle() : 0; style = style
	 * == 2 ? 1 : style == 3 ? 3 : 0; int defLvl = player ? ((Player)
	 * e).getSkills().getLevel(SkillType.DEFENCE) : (int) (((NPC)
	 * e).getCombatLevel() * 0.6); int defBonus = player ? ((Player)
	 * e).getContent().get(ContentType.COMBATDEF).getBonuses()[
	 ...
	 * e.getSkills().getLevel(SkillType.RANGE); int attBonus =
	 * e.getContent().get(ContentType.COMBATDEF).getBonuses()[4]; double attMult = 1.0 *
	 * e.getContent().get(ContentKind.PRAYER).getRangeMultiplier() *
	 * e.getContent().get(AbstractContentType.AURAS).getRangeAccurayMultiplier(); double accuracyMultiplier
	 ...
	 * e.getSkills().getLevel(SkillType.RANGE); int strBonus =
	 * e.getCombatDefinitions
	 * ().getBonuses()[CombatDefinitions.RANGED_STR_BONUS]; double strMult = 1.0
	 * * e.getContent().get(ContentKind.PRAYER).getRangeMultiplier(); double hitMultiplier = 1.0; if
	 ...
	 * e).getContent().get(ContentType.COMBATDEF).getAttackStyle() : 0; style = style == 2 ? 1 :
	 * style == 3 ? 3 : 0; int defLvl = player ? ((Player)
	 * e).getSkills().getLevel(SkillType.DEFENCE) : (int) (((NPC)
	 * e).getCombatLevel() * 0.6); int defBonus = player ? ((Player)
	 * e).getContent().get(ContentType.COMBATDEF).getBonuses()[CombatDefinitions.RANGE_DEF] : 0;
	...
	private double getSpecialAccuracyModifier(Player player) {
		Item weapon = player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_WEAPON);
		if (weapon == null)
			return 1;
	...
	public static final boolean fullVanguardEquipped(Player player) {
		int helmId = player.getContent().get(ContentType.EQUIPMENT).getHatId();
		int chestId = player.getContent().get(ContentType.EQUIPMENT).getChestId();
		int legsId = player.getContent().get(ContentType.EQUIPMENT).getLegsId();
		int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
		int bootsId = player.getContent().get(ContentType.EQUIPMENT).getBootsId();
		int glovesId = player.getContent().get(ContentType.EQUIPMENT).getGlovesId();
	...
	public static final boolean usingGoliathGloves(Player player) {
		String name = player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_SHIELD) != null ? player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_SHIELD).getDefinitions().getName().toLowerCase() : "";
		if (player.getContent().get(ContentType.EQUIPMENT).getItem((Equipment.SLOT_HANDS)) != null) {
			if (player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_HANDS).getDefinitions().getName().toLowerCase().contains("goliath") && player.getContent().get(ContentType.EQUIPMENT).getWeaponId() == -1) {
	...
	public static final boolean fullVoidEquipped(Player player, int... helmid) {
		boolean hasDeflector = player.getContent().get(ContentType.EQUIPMENT).getShieldId() == 19712;
		if (player.getContent().get(ContentType.EQUIPMENT).getGlovesId() != 8842) {
			if (hasDeflector)
				hasDeflector = false;
			else
				return false;
		...
		int torsoId = player.getContent().get(ContentType.EQUIPMENT).getChestId();
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
		...
		final MobileEntity target = this.target;
		...
		for (Hit hit : hits) {
			...
			if (target instanceof Player) {
				Player p2 = (Player) target;
				if (player.getContent().get(ContentType.PRAYER).usingPrayer(1, 18))
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				...
				if (combatXp > 0) {
					player.getContent().get(ContentType.AURAS).checkSuccefulHits(hit.getDamage());
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						if (attackStyle == 2) {
							player.getSkills().addXp(SkillType.RANGE, combatXp / 2);
							player.getSkills().addXp(SkillType.DEFENCE, combatXp / 2);
						} else
							player.getSkills().addXp(SkillType.RANGE, combatXp);

					} else {
						SkillType xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
						if (xpStyle.getSkillId() != CombatDefinitions.SHARED)
							player.getSkills().addXp(xpStyle, combatXp);
						else {
							player.getSkills().addXp(SkillType.ATTACK, combatXp / 3);
							player.getSkills().addXp(SkillType.STRENGTH, combatXp / 3);
							player.getSkills().addXp(SkillType.DEFENCE, combatXp / 3);
						}
					}
					double hpXp = damage / 7.5;
					if (hpXp > 0)
						player.getSkills().addXp(SkillType.HITPOINTS, hpXp);
				}
				...
				if (combatXp > 0) {
					player.getContent().get(ContentType.AURAS).checkSuccefulHits(hit.getDamage());
					if (player.getContent().get(ContentType.COMBATDEF).isDefensiveCasting() || (hasPolyporeStaff(player) && player.getContent().get(ContentType.COMBATDEF).getAttackStyle() == 1)) {
						int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(SkillType.DEFENCE, defenceXp / 7.5);
						}
					}
					player.getSkills().addXp(SkillType.MAGIC, combatXp);
					double hpXp = damage / 7.5;
					if (hpXp > 0)
						player.getSkills().addXp(SkillType.HITPOINTS, hpXp);
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (Hit hit : hits) {
						...
						if (combatXp > 0) {
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								if (weaponId != -1) {
									...
									if (name.contains("(p++)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 48);
									} else if (name.contains("(p+)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 38);
									} else if (name.contains("(p)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 28);
									}
								}
							} else {
								if (weaponId != -1) {
									...
									if (name.contains("(p++)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 68);
									} else if (name.contains("(p+)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 58);
									} else if (name.contains("(p)")) {
										if (Utilities.getRandom(8) == 0)
											target.getPoison().makePoisoned(player, 48);
									}
						...
						if (splash) {
							...
							if (mage_hit_gfx != 0) {
								...
								if (block_tele) {
									if (target instanceof Player) {
										Player targetPlayer = (Player) target;
										targetPlayer.setTeleBlockDelay((targetPlayer.getContent().get(ContentType.PRAYER).usingPrayer(0, 17) || targetPlayer.getContent().get(ContentType.PRAYER).usingPrayer(1, 7) ? 100000 : 300000));
					...
					if (max_poison_hit > 0 && Utilities.getRandom(10) == 0) {
						if (!target.getPoison().isPoisoned())
							target.getPoison().makePoisoned(player, max_poison_hit);
					}
					if (target instanceof Player) {
						Player p2 = (Player) target;
						p2.closeInterfaces();
						if (p2.getContent().get(ContentType.COMBATDEF).isAutoRelatie() && !p2.getContent().get(ContentType.ACTION).actionIsRunning() && !p2.hasWalkSteps())
							p2.getContent().get(ContentType.ACTION).setAction(new PlayerCombat(player));
					} else {
						NPC n = (NPC) target;
	...
	private boolean checkAll(Player player) {
		...
		if (target instanceof Player) {
			...
			if (n.isCantInteract()) {
				...
				if (n.getId() == 14578) {
					if (player.getContent().get(ContentType.EQUIPMENT).getWeaponId() != 2402 && player.getContent().get(ContentType.COMBATDEF).getAutoCastSpell() <= 0 && !hasPolyporeStaff(player)) {
		...
		if (!(target instanceof NPC && ((NPC) target).isForceMultiAttacked())) {
			...
			if (!target.isAtMultiArea() || !player.isAtMultiArea()) {
				if (player.getAttackedBy() != target && player.getAttackedByDelay() > Utilities.currentTimeMillis()) {
					return false;
				}
				if (target.getAttackedBy() != player && target.getAttackedByDelay() > Utilities.currentTimeMillis()) {
					return false;
				}
			}
		}
		...
		if (distanceX < size && distanceX > -1 && distanceY < size && distanceY > -1 && !target.hasWalkSteps()) {
			...
			return true;
		} else if (isRanging == 0 && target.getSize() == 1 && player.getContent().get(ContentType.COMBATDEF).getSpellId() <= 0 && !hasPolyporeStaff(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
			if (!player.addWalkSteps(target.getX(), player.getY(), 1))
				player.addWalkSteps(player.getX(), target.getY(), 1);
			return true;
		}
		maxDistance = isRanging != 0 || player.getContent().get(ContentType.COMBATDEF).getSpellId() > 0 || hasPolyporeStaff(player) ? 7 : 0;
		...
		if (player.getContent().get(ContentType.COMBATDEF).isInstantAttack()) {
			player.getContent().get(ContentType.COMBATDEF).setInstantAttack(false);
			if (player.getContent().get(ContentType.COMBATDEF).getAutoCastSpell() > 0)
				return true;
			if (player.getContent().get(ContentType.COMBATDEF).isUsingSpecialAttack()) {
				if (!specialExecute(player))
					return true;
				player.getContent().get(ContentType.ACTION).setActionDelay(0);
				int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
				int attackStyle = player.getContent().get(ContentType.COMBATDEF).getAttackStyle();
				switch (weaponId) {
				case 4153:
				...
				player.getContent().get(ContentType.ACTION).setActionDelay(4);
			}
			return true;
		}
		return true;
	...
	public static final int isRanging(Player player) {
		...
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart") || name.contains("javelin") || name.contains("thrownaxe") || name.contains("throwing axe") || name.contains("crystal bow") || name.equalsIgnoreCase("zaryte bow") || name.contains("chinchompa") || name.contains("Bolas"))
				return 2;
		}
		int ammoId = player.getContent().get(ContentType.EQUIPMENT).getAmmoId();
		switch (weaponId) {
