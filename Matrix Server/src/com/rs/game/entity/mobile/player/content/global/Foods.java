...
public class Foods {
	...
	public static enum Food {
		...
		SLIMY_EEL(3381, 7 + Utilities.random(2)),

		RAINBOW_FISH(10136, 11),

		CAVE_EEL(5003, 8 + Utilities.random(2)),

		LAVA_EEL(2149, 7 + Utilities.random(2)),
		...
		THIN_SNAIL_MEAT(3369, 5 + Utilities.random(2)),

		LEAN_SNAIL_MEAT(3371, 8),

		FAT_SNAIL_MEAT(3373, 8 + Utilities.random(2));
	...
	public static enum Effect {
		SUMMER_PIE {
			...
			public void effect(Object object) {
				...
				int level = player.getSkills().getLevel(SkillType.AGILITY);
				int realLevel = player.getSkills().getLevelForXp(SkillType.AGILITY);
				player.getSkills().setLevelBoost(SkillType.AGILITY, level >= realLevel ? realLevel + 5 : level + 5);
			}
		...
		GARDEN_PIE {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				int level = player.getSkills().getLevel(SkillType.FARMING);
				int realLevel = player.getSkills().getLevelForXp(SkillType.FARMING);
				player.getSkills().setLevelBoost(SkillType.FARMING, level >= realLevel ? realLevel + 3 : level + 3);
			}
		...
		FISH_PIE {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				int level = player.getSkills().getLevel(SkillType.FISHING);
				int realLevel = player.getSkills().getLevelForXp(SkillType.FISHING);
				player.getSkills().setLevelBoost(SkillType.FISHING, level >= realLevel ? realLevel + 3 : level + 3);
			}
		},
		...
		ADMIRAL_PIE {
			...
			public void effect(Object object) {
				...
				int level = player.getSkills().getLevel(SkillType.FISHING);
				int realLevel = player.getSkills().getLevelForXp(SkillType.FISHING);
				player.getSkills().setLevelBoost(SkillType.FISHING, level >= realLevel ? realLevel + 5 : level + 5);
			}
		},
		...
		WILD_PIE {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				int level = player.getSkills().getLevel(SkillType.SLAYER);
				int realLevel = player.getSkills().getLevelForXp(SkillType.SLAYER);
				player.getSkills().setLevelBoost(SkillType.SLAYER, level >= realLevel ? realLevel + 4 : level + 4);
				int level2 = player.getSkills().getLevel(SkillType.RANGE);
				int realLevel2 = player.getSkills().getLevelForXp(SkillType.RANGE);
				player.getSkills().setLevelBoost(SkillType.RANGE, level2 >= realLevel2 ? realLevel2 + 4 : level2 + 4);
			}
		},
		...
		SPICY_STEW_EFFECT {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				if (Utilities.random(100) > 5) {
					int level = player.getSkills().getLevel(SkillType.COOKING);
					int realLevel = player.getSkills().getLevelForXp(SkillType.COOKING);
					player.getSkills().setLevelBoost(SkillType.COOKING, level >= realLevel ? realLevel + 6 : level + 6);
				} else {
					int level = player.getSkills().getLevel(SkillType.COOKING);
					player.getSkills().setLevelBoost(SkillType.COOKING, level <= 6 ? 0 : level - 6);
				}
			}
		...
		ONION_MESSAGE {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.getPackets().sendGameMessage("It hurts to see a grown " + player.getContent().get(ContentType.APPEARANCE).isMale() != null ? "male" : "female" + "cry.");
			}
		},
	...
	public static boolean eat(final Player player, Item item, int slot) {
		...
		if (food == null)
			return false;
		if (player.getFoodDelay() > Utilities.currentTimeMillis())
			return true;
		...
		player.getContent().get(ContentType.ACTION).setActionDelay((int) foodDelay / 1000);
		player.addFoodDelay(foodDelay);
		player.getContent().get(ContentType.ACTION).setActionDelay(player.getContent().get(ContentType.ACTION).getActionDelay() + 2);
		player.getContent().get(ContentType.INVENTORY).getItems().set(slot, food.getNewId() == 0 ? null : new Item(food.getNewId(), 1));
		...
		player.getContent().get(ContentType.INVENTORY).refresh();
		if (food.effect != null) {
