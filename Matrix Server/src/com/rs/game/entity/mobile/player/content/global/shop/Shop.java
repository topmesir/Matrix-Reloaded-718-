package com.rs.game.entity.mobile.player.content.global.shop;
...
import com.rs.utilities.ItemSetsKeyGenerator;
...
public class Shop {
	...
	public void sendInventory(Player player) {
		...
		player.getPackets().sendItems(93, player.getContent().get(ContentType.INVENTORY).getItems());
	...
	public void buy(Player player, int clickSlot, int quantity) {
		...
		int amountCoins = player.getContent().get(ContentType.INVENTORY).getItems().getNumberOf(money);
		...
		if (item.getDefinitions().isStackable()) {
			if (player.getContent().get(ContentType.INVENTORY).getFreeSlots() < 1) {
				player.getPackets().sendGameMessage(
		...
		if (buyQ != 0) {
			...
			player.getContent().get(ContentType.INVENTORY).deleteItem(money, totalPrice);
			player.getContent().get(ContentType.INVENTORY).addItem(item.getId(), buyQ);
	...
	public void sell(Player player, int slotId, int quantity) {
		if (player.getContent().get(ContentType.INVENTORY).getItemsContainerSize() < slotId)
			return;
		...
		int numberOff = player.getContent().get(ContentType.INVENTORY).getItems()
		...
		player.getContent().get(ContentType.INVENTORY).deleteItem(originalId, quantity);
		player.getContent().get(ContentType.INVENTORY).addItem(money, price * quantity);
	}
			...
			if (requiriments != null && !requiriments.isEmpty()) {
				...
				for (SkillType skillId : requiriments.keySet()) {
					...
					reqsText += "<br>"
							...
							+ "Level " + level + " "
							+ Skills.SKILL_NAME[skillId.getSkillId()];
				}
