...
public final class CombatDefinitions extends AbstractContent {
	...
	public static final SkillType getXpStyle(int weaponId, int attackStyle) {
	...
	public void refreshBonuses() {
		bonuses = new int[18];
		for (Item item : player.getContent().get(ContentType.EQUIPMENT).getItems().getItems()) {
			if (item == null)
				continue;
	...
	public boolean hasRingOfVigour() {
		return player.getContent().get(ContentType.EQUIPMENT).getRingId() == 19669;
	}
	...
	public static boolean hasAntiFireShield(MobileEntity entity) {
		Player player = null;
		if(entity instanceof Player)
			player = (Player) entity;
		else return false;
		Item item = new Item(player.getContent().get(ContentType.EQUIPMENT).getShieldId());
		if(item.getDefinitions() == null)
			return false;
		String name = item.getDefinitions().getName().toLowerCase();
		if(name.contains("dragon fire"))
			return true;
		return false;
	}
