...
public class FriendsList extends AbstractContent {
	...
	public void changeFriendStatus(Player p2, boolean online) {
		...
		if (online && !isOnline(p2))
			online = false;
		player.getPackets().sendFriend(Utilities.formatPlayerNameForDisplay(p2.getUsername()), p2.getDisplayName(), 1, online, true);
	}
	...
	public void sendFriendsMyStatus(boolean online) {
		for (Player p2 : World.getPlayers()) {
			if (p2 == null || !p2.hasStarted() || p2.hasFinished())
				continue;
			p2.getContent().get(ContentType.FRIENDSLIST).changeFriendStatus(player, online);
		}
	}
	...
	public void sendMessage(Player p2, String message) {
		...
		p2.getPackets().receivePrivateMessage(Utilities.formatPlayerNameForDisplay(player.getUsername()), player.getDisplayName(), player.getMessageIcon(), message);
	}
	...
	public void sendQuickChatMessage(Player p2, QuickChatMessage quickChatMessage) {
		...
		p2.getPackets().receivePrivateChatQuickMessage(Utilities.formatPlayerNameForDisplay(player.getUsername()), player.getDisplayName(), player.getMessageIcon(), quickChatMessage);
	...
	public void changeRank(String username, int rank) {
		...
		player.getPackets().sendFriend(Utilities.formatPlayerNameForDisplay(username), displayName, 1, online, true);
		FriendsChatManager.refreshChat(player);
	}
			...
			if (componentId == 1) {
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					if (chatName != null) {
						chatName = null;
						refreshChatName();
						FriendsChatManager.destroyChat(player);
					}
				}
	...
	public void addIgnore(String username, boolean tillLogout) {
		...
		Player p2 = World.getPlayerByDisplayName(username);
		String formatedUsername = p2 != null ? p2.getUsername() : Utilities.formatPlayerNameForProtocol(username);
		...
		player.getPackets().sendIgnore(Utilities.formatPlayerNameForDisplay(p2 != null ? p2.getUsername() : username), p2 != null ? p2.getDisplayName() : Utilities.formatPlayerNameForDisplay(username), false);
	}
	...
	public void addFriend(String username) {
		...
		FriendsChatManager.refreshChat(player);
		boolean online = p2 != null && isOnline(p2);
		player.getPackets().sendFriend(Utilities.formatPlayerNameForDisplay(username), displayName, 1, online, online);
		if (privateStatus == 1 && p2 != null)
			p2.getContent().get(ContentType.FRIENDSLIST).changeFriendStatus(player, true);
	}
	...
	public boolean isOnline(Player p2) {
		if (p2.getContent().get(ContentType.FRIENDSLIST).privateStatus == 2)
			return false;
		if (p2.getContent().get(ContentType.FRIENDSLIST).privateStatus == 1 && !p2.getContent().get(ContentType.FRIENDSLIST).friends.contains(player.getUsername()))
			return false;
		return true;
	}
	...
	public void init() {
		...
		if (privateStatus != 2)
			sendFriendsMyStatus(true);
		if (hasFriendChat())
			FriendsChatManager.linkSettings(player);
	}
