package com.rs.game.entity.mobile.player.content.type.container;
...
public class Bank extends AbstractContent {
	...
	public void depositAllInventory(boolean banking) {
		if (Bank.MAX_BANK_SIZE - getBankSize() < player.getContent().get(ContentType.INVENTORY).getItems().getSize()) {
	...
	public void depositAllEquipment(boolean banking) {
		int space = addItems(player.getContent().get(ContentType.EQUIPMENT).getItems().getItems(), banking);
		if (space != 0) {
			for (int i = 0; i < space; i++)
				player.getContent().get(ContentType.EQUIPMENT).getItems().set(i, null);
			player.getContent().get(ContentType.EQUIPMENT).init();
			player.getContent().get(ContentType.APPEARANCE).generateAppearenceData();
		}
		if (space < player.getContent().get(ContentType.EQUIPMENT).getItems().getSize()) {
			player.getPackets().sendGameMessage("Not enough space in your bank.");
			return;
		}
	}
	...
	public void withdrawItem(int bankSlot, int quantity) {
		...
		if (noted || defs.isStackable()) {
			if (player.getContent().get(ContentType.INVENTORY).getItems().containsOne(item)) {
				int slot = player.getContent().get(ContentType.INVENTORY).getItems().getThisItemSlot(item);
				Item invItem = player.getContent().get(ContentType.INVENTORY).getItems().get(slot);
				if (invItem.getAmount() + item.getAmount() <= 0) {
					item.setAmount(Integer.MAX_VALUE - invItem.getAmount());
					player.getPackets().sendGameMessage("Not enough space in your inventory.");
				}
			} else if (!player.getContent().get(ContentType.INVENTORY).hasFreeSlots()) {
				player.getPackets().sendGameMessage("Not enough space in your inventory.");
				return;
			}
		} else {
	...
	public void depositItem(int invSlot, int quantity, boolean refresh) {
		if (quantity < 1 || invSlot < 0 || invSlot > 27)
			return;
		Item item = player.getContent().get(ContentType.INVENTORY).getItem(invSlot);
		if (item == null)
			return;
		int amt = player.getContent().get(ContentType.INVENTORY).getItems().getNumberOf(item);
		if (amt < quantity)
		...
		player.getContent().get(ContentType.INVENTORY).deleteItem(invSlot, new Item(originalId, item.getAmount()));
