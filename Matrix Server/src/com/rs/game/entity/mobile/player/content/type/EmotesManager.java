...
public final class EmotesManager extends AbstractContent {
	...
	public void useBookEmote(int id) {
		...
		if (!unlockedEmotes.contains(id)) {
			if (id == 41)
				player.getDialogueManager().startDialogue("SimpleMessage", "This emote can be acessed by unlocking " + Settings.AIR_GUITAR_MUSICS_COUNT + " pieces of music.");
			else
				player.getDialogueManager().startDialogue("SimpleMessage", "You need to unlock this emote by yourself.");
		} else {
			if (Utilities.currentTimeMillis() < nextEmoteEnd) {
			...
			if (id == 2) {
				...
				// TODO skillCape
				final int capeId = player.getContent().get(ContentType.EQUIPMENT).getCapeId();
				switch (capeId) {
				case 9747:
				case 9748:
				...
				case 18509: // Dungeoneering cape
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if (step == 1) {
								player.getContent().get(ContentType.APPEARANCE).transformIntoNPC((rand == 0 ? 11227 : (rand == 1 ? 11228 : 11229)));
								player.setNextAnimation(new Animation(((rand > 0 ? 13192 : (rand == 1 ? 13193 : 13194)))));
							}
							if (step == 3) {
								player.getContent().get(ContentType.APPEARANCE).transformIntoNPC(-1);
								stop();
							}
							step++;
						}
				...
				case 19710: // Master dungeoneering cape
					 ...
					 * player.getContent().get(AbstractContentType.APPEARANCE).transformIntoNPC(11229);
					 ...
					 * player.getContent().get(AbstractContentType.APPEARANCE).transformIntoNPC(11228);
					 ...
					 * dung2.finish();
					 * player.getContent().get(AbstractContentType.APPEARANCE).transformIntoNPC(11227);
					 ...
					 * player.getContent().get(AbstractContentType.APPEARANCE).transformIntoNPC(-1);
				...
				case 20765: // Classic cape
					...
					int random = Utilities.getRandom(2);
				...
				case 20767: // Max cape
					...
					nextEmoteEnd = Utilities.currentTimeMillis() + (25 * 600);
				...
				case 20771: // Compl cape
					...
					nextEmoteEnd = Utilities.currentTimeMillis() + (20 * 600);
					WorldTasksManager.schedule(new WorldTask() {
						private int step;
						...
						public void run() {
							if (step == 0) {
								...
								player.getContent().get(ContentType.APPEARANCE).transformIntoNPC(capeId == 20769 ? 1830 : 3372);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (step == 0) {
							...
							player.getContent().get(ContentType.APPEARANCE).transformIntoNPC(8499);
						} else if (step == 6) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (step == 0) {
							...
							player.getContent().get(ContentType.APPEARANCE).transformIntoNPC(random == 0 ? 13255 : (random == 1 ? 13256 : 13257));
						} else if (step == 2) {
	...
	public void setNextEmoteEnd(long delay) {
		nextEmoteEnd = Utilities.currentTimeMillis() + delay;
	}
