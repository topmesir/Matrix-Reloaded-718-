...
public class AuraManager extends AbstractContent {
	...
	public void process() {
		if (!isActivated())
			return;
		if (activation - Utilities.currentTimeMillis() <= 60000 && !warned) {
	...
	public void activate() {
		Item item = player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_AURA);
		if (item == null)
			return;
		...
		if (toId != -1) {
			player.getContent().get(ContentType.EQUIPMENT).getItem(Equipment.SLOT_AURA).setId(toId);
			player.getContent().get(ContentType.EQUIPMENT).refresh(Equipment.SLOT_AURA);
			player.getContent().get(ContentType.APPEARANCE).generateAppearenceData();
		} else {
			if (activation != 0) {
			...
			if (Utilities.currentTimeMillis() <= getCoolDown(item.getId())) {
				// TODO message cooldown
				return;
			}
			int tier = getTier(item.getId());
			activation = Utilities.currentTimeMillis() + getActivationTime(item.getId()) * 1000;
	...
	public void sendAuraRemainingTime() {
		if (!isActivated()) {
			long cooldown = getCoolDown(player.getContent().get(ContentType.EQUIPMENT).getAuraId());
			if (Utilities.currentTimeMillis() <= cooldown) {
				player.getPackets().sendGameMessage("Currently recharging. <col=ff0000>" + getFormatedTime((cooldown - Utilities.currentTimeMillis()) / 1000) + " remaining.");
				return;
			}
			player.getPackets().sendGameMessage("Currently desactivate. It is ready to use.");
			return;
		}
		player.getPackets().sendGameMessage("Currently active. <col=00ff00>" + getFormatedTime((activation - Utilities.currentTimeMillis()) / 1000) + " remaining");
	}
	...
	public void sendTimeRemaining(int aura) {
		...
		if (cooldown < Utilities.currentTimeMillis()) {
	...
	public int getAuraModelId2() {
		int aura = player.getContent().get(ContentType.EQUIPMENT).getAuraId();
		switch (aura) {
	...
	public void useInspiration() {
		...
		if (atts == 5) {
			atts = 0;
			player.getContent().get(ContentType.COMBATDEF).restoreSpecialAttack(1);
		}
