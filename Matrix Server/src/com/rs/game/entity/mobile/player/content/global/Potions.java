...
public final class Potions {
	...
	public static enum Pot {
		...
		MAGIC_FLASK(new int[] { 23423, 23425, 23427, 23429, 23431, 23433 }, Effects.MAGIC_POTION), ANTI_POISION(new int[] { 2446, 175, 177, 179 }, Effects.ANTIPOISON),
	...
	private enum Effects {
		ATTACK_POTION(SkillType.ATTACK) {
			@Override
		...
		FISHING_POTION(SkillType.FISHING) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (level + 3);
		...
		ZAMORAK_BREW(SkillType.ATTACK) {
			int toAdd;

			@Override
			public void extra(Player player) {
				toAdd = (player.getSkills().getLevelForXp(SkillType.ATTACK));
				player.getSkills().setLevelForXp(SkillType.ATTACK, toAdd);
			}

		},
		SANFEW_SERUM(SkillType.ATTACK, SkillType.STRENGTH, SkillType.DEFENCE, SkillType.MAGIC, SkillType.RANGE, SkillType.AGILITY, SkillType.COOKING, SkillType.CRAFTING, SkillType.FARMING, SkillType.FIREMAKING, SkillType.FISHING, SkillType.FLETCHING, SkillType.HERBLORE, SkillType.MINING, SkillType.RUNECRAFTING, SkillType.SLAYER, SkillType.SMITHING, SkillType.THIEVING, SkillType.WOODCUTTING, SkillType.SUMMONING) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
			...
			@Override
			public void extra(Player player) {
				player.getContent().get(ContentType.PRAYER).restorePrayer((int) ((int) (player.getSkills().getLevelForXp(SkillType.PRAYER) * 0.33 * 10) * player.getContent().get(ContentType.AURAS).getPrayerPotsRestoreMultiplier()));
				player.addPoisonImmune(180000);
				// TODO DISEASE HEALING
			}

		},
		SUMMONING_POT(SkillType.SUMMONING) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int restore = (int) (Math.floor(player.getSkills().getLevelForXp(SkillType.SUMMONING) * 0.25) + 7);
		...
		STRENGTH_POTION(SkillType.STRENGTH) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 3 + (realLevel * 0.1));
		...
		DEFENCE_POTION(SkillType.DEFENCE) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 3 + (realLevel * 0.1));
		...
		PRAYER_POTION() {
			@Override
			public void extra(Player player) {
				player.getContent().get(ContentType.PRAYER).restorePrayer((int) ((int) (Math.floor(player.getSkills().getLevelForXp(SkillType.PRAYER) * 2.5) + 70) * player.getContent().get(ContentType.AURAS).getPrayerPotsRestoreMultiplier()));
			}
		},
		SUPER_STR_POTION(SkillType.STRENGTH) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
		...
		SUPER_DEF_POTION(SkillType.DEFENCE) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 5 + (realLevel * 0.15));
		...
		SUPER_ATT_POTION(SkillType.ATTACK) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 5 + (realLevel * 0.15));
		...
		EXTREME_STR_POTION(SkillType.STRENGTH) {
		...
		EXTREME_DEF_POTION(SkillType.DEFENCE) {
		...
		EXTREME_ATT_POTION(SkillType.ATTACK) {
		...
		EXTREME_RAN_POTION(SkillType.RANGE) {
		...
		EXTREME_MAG_POTION(SkillType.MAGIC) {
		...
		RECOVER_SPECIAL() {
			...
			public boolean canDrink(Player player) {
				...
				if (time != null && Utilities.currentTimeMillis() - time < 30000) {
			...
			@Override
			public void extra(Player player) {
				player.getTemporaryAttributtes().put("Recover_Special_Pot", Utilities.currentTimeMillis());
				player.getContent().get(ContentType.COMBATDEF).restoreSpecialAttack(25);
			}
		},
		SARADOMIN_BREW("You drink some of the foul liquid.", SkillType.ATTACK, SkillType.DEFENCE, SkillType.STRENGTH, SkillType.MAGIC, SkillType.RANGE) {
			...
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				if (skillId == SkillType.DEFENCE.getSkillId()) {
		...
		SUPER_PRAYER() {
			@Override
			public void extra(Player player) {
				player.getContent().get(ContentType.PRAYER).setPrayerpoints((int) ((int) (70 + (player.getSkills().getLevelForXp(SkillType.PRAYER) * 3.43)) * player.getContent().get(ContentType.AURAS).getPrayerPotsRestoreMultiplier()));
			}
		},
		PRAYER_RENEWAL() {
			@Override
		...
		SUPER_RESTORE(SkillType.ATTACK, SkillType.STRENGTH, SkillType.DEFENCE, SkillType.MAGIC, SkillType.RANGE, SkillType.AGILITY, SkillType.COOKING, SkillType.CRAFTING, SkillType.FARMING, SkillType.FIREMAKING, SkillType.FISHING, SkillType.FLETCHING, SkillType.HERBLORE, SkillType.MINING, SkillType.RUNECRAFTING, SkillType.SLAYER, SkillType.SMITHING, SkillType.THIEVING, SkillType.WOODCUTTING, SkillType.SUMMONING) {
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int boost = (int) (realLevel * 0.33);
				if (actualLevel > realLevel)
		...
		RESTORE_POTION(SkillType.ATTACK, SkillType.STRENGTH, SkillType.MAGIC, SkillType.RANGE, SkillType.PRAYER) {
		...
		private SkillType[] affectedSkills;
		private String drinkMessage;

		private Effects(SkillType... affectedSkills) {
			this(null, affectedSkills);
		}

		private Effects(String drinkMessage, SkillType... affectedSkills) {
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		if (pot == null)
			return false;
		if (player.getPotDelay() > Utilities.currentTimeMillis())
			return true;
		...
		if (name.contains("flask") && dosesLeft <= 0 && toPot >= pot.id.length) {
			player.getContent().get(ContentType.INVENTORY).deleteItem(slot, new Item(pot.id[5]));
		} else
			player.getContent().get(ContentType.INVENTORY).getItems().set(slot, new Item(dosesLeft > 0 && toPot < pot.id.length ? pot.id[toPot] : 229, 1));
		player.getContent().get(ContentType.INVENTORY).refresh(slot);
		for (SkillType skillId : pot.effect.affectedSkills)
			player.getSkills().setLevelForXp(skillId, pot.effect.getAffectedSkill(player, skillId.getSkillId(), player.getSkills().getLevel(skillId), player.getSkills().getLevelForXp(skillId)));
		pot.effect.extra(player);
		player.setNextAnimation(new Animation(829));
		player.getPackets().sendSound(4580, 0, 1);
		player.getPackets().sendGameMessage(pot.effect.drinkMessage != null ? pot.effect.drinkMessage : "You drink some of your " + name.toLowerCase().replace(" (1)", "").replace(" (2)", "").replace(" (3)", "").replace(" (4)", "").replace(" (5)", "").replace(" (6)", "") + ".", true);

		player.getPackets().sendGameMessage(dosesLeft == 0 ? "You have finished your " + (name.contains("flask") ? "flask and the glass shatters to peices." : "potion.") : "You have " + dosesLeft + " dose of potion left.", true);
		return true;
	}

	public static boolean mixPotion(Player player, int usedSlot, int withSlot) {
		/*
		 * Item itemUsed = player.getContent().get(ContentType.INVENTORY).getItem(usedSlot); Item
		 * usedWith = player.getContent().get(ContentType.INVENTORY).getItem(withSlot); Pot first =
	...
	public static void resetOverLoadEffect(Player player) {
		SkillType[] skillsToSet = new SkillType[] {
				SkillType.ATTACK, SkillType.STRENGTH, SkillType.DEFENCE, SkillType.MAGIC, SkillType.RANGE
		};
		if (!player.isDead()) {
			for(SkillType skill : skillsToSet) {
				int actualLevel = player.getSkills().getLevel(skill);
				int realLevel = player.getSkills().getLevelForXp(skill);
				if (actualLevel > realLevel)
					player.getSkills().setLevelForXp(skill, realLevel);
			}
			player.heal(500);
		}
	...
	public static void applyOverLoadEffect(Player player) {
		...
		for(SkillType skill : skillsToSet) {
			boolean inWild = player.getControlerManager().getControler() instanceof Wilderness;
			int actualLevel = player.getSkills().getLevel(skill);
			int realLevel = player.getSkills().getLevelForXp(skill);
			int level = Math.min(realLevel, actualLevel);
			int xp = level + (skill == SkillType.RANGE ? 4 : skill == SkillType.MAGIC ? 7 : 5);
			int endXp = (int) (skill == SkillType.RANGE ? (inWild ? (realLevel * 0.1) 
					: Math.floor(realLevel / 5.2)) : skill == SkillType.MAGIC ? 0 
							: (inWild ? (realLevel * 0.15) : (realLevel * 0.22)));
			player.getSkills().setLevelForXp(skill, xp + endXp);
		}
	}
