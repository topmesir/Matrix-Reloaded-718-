...
public class Magic {
	...
	private static final int AIR_RUNE = 556, WATER_RUNE = 555, EARTH_RUNE = 557, FIRE_RUNE = 554, MIND_RUNE = 558, NATURE_RUNE = 561, CHAOS_RUNE = 562, DEATH_RUNE = 560, BLOOD_RUNE = 565, SOUL_RUNE = 566, ASTRAL_RUNE = 9075, LAW_RUNE = 563, STEAM_RUNE = 4694, MIST_RUNE = 4695, DUST_RUNE = 4696, SMOKE_RUNE = 4697, MUD_RUNE = 4698, LAVA_RUNE = 4699, ARMADYL_RUNE = 21773;
	...
	public static final boolean checkCombatSpell(Player player, int spellId, int set, boolean delete) {
		if (spellId == 65535)
			return true;
		switch (player.getContent().get(ContentType.COMBATDEF).getSpellBook()) {
		case 193:
			switch (spellId) {
			case 28:
			...
			case 38: // Miasmic burst.
				...
				weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
				if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941 && weaponId != 13943) {
					player.getPackets().sendGameMessage("You need a Zuriel's staff to cast this spell.");
					player.getPackets().sendGameMessage("Extreme donators can cast Miasmic spells without Zuriel's staff.");
					return false;
		...
		case 192:
			...
			case 89:
				...
				break;
			case 66: // Sara Strike
				if (player.getContent().get(ContentType.EQUIPMENT).getWeaponId() != 2415) {
			...
			case 67: // Guthix Claws
				if (player.getContent().get(ContentType.EQUIPMENT).getWeaponId() != 2416) {
			...
			case 68: // Flame of Zammy
				if (player.getContent().get(ContentType.EQUIPMENT).getWeaponId() != 2417) {
		...
		if (set >= 0) {
			if (set == 0)
				player.getContent().get(ContentType.COMBATDEF).setAutoCastSpell(spellId);
			else
	...
	public static final void setCombatSpell(Player player, int spellId) {
		if (player.getContent().get(ContentType.COMBATDEF).getAutoCastSpell() == spellId)
	...
	public static final void processLunarSpell(Player player, int spellId, int packetId) {
		...
		switch (spellId) {
		case 37:
			if (player.getSkills().getLevel(SkillType.MAGIC) < 94) {
				player.getPackets().sendGameMessage("Your Magic level is not high enough for this spell.");
				return;
			} else if (player.getSkills().getLevel(SkillType.DEFENCE) < 40) {
			...
			if (lastVeng != null && lastVeng + 30000 > Utilities.currentTimeMillis()) {
			...
			player.getTemporaryAttributtes().put("LAST_VENG", Utilities.currentTimeMillis());
	...
	public static final void processNormalSpell(Player player, int spellId, int packetId) {
		...
		case 27: // crossbow bolt enchant
			if (player.getSkills().getLevel(SkillType.MAGIC) < 4) {
				player.getPackets().sendGameMessage("Your Magic level is not high enough for this spell.");
				return;
			}
			player.stopAll();
	...
	public static final boolean checkSpellRequirements(Player player, int level, boolean delete, int... runes) {
		if (player.getSkills().getLevelForXp(SkillType.MAGIC) < level) {
			player.getPackets().sendGameMessage("Your Magic level is not high enough for this spell.");
			return false;
		}
	...
	public static final boolean checkRunes(Player player, boolean delete, int... runes) {
		int weaponId = player.getContent().get(ContentType.EQUIPMENT).getWeaponId();
		int shieldId = player.getContent().get(ContentType.EQUIPMENT).getShieldId();
		int runesCount = 0;
		while (runesCount < runes.length) {
			...
			if (hasInfiniteRunes(runeId, weaponId, shieldId))
				continue;
			if (hasStaffOfLight(weaponId) && Utilities.getRandom(8) == 0 && runeId != 21773)// 1
																						// in
																						// eight
																						// chance
																						// of
																						...
																						// runes
				continue;
			if (!player.getContent().get(ContentType.INVENTORY).containsItem(runeId, ammount)) {
		...
		if (delete) {
			...
			while (runesCount < runes.length) {
				...
				if (hasInfiniteRunes(runeId, weaponId, shieldId))
					continue;
				player.getContent().get(ContentType.INVENTORY).deleteItem(runeId, ammount);
			}
		}
		return true;
	}
	...
	public static final boolean sendTeleportSpell(final Player player, int upEmoteId, final int downEmoteId, int upGraphicId, final int downGraphicId, int level, final double xp, final WorldTile tile, int delay, final boolean randomize, final int teleType, int... runes) {
		...
		if (player.getLockDelay() > currentTime)
			return false;
		if (player.getSkills().getLevel(SkillType.MAGIC) < level) {
			player.getPackets().sendGameMessage("Your Magic level is not high enough for this spell.");
			return false;
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (!removeDamage) {
					...
					if (xp != 0)
						player.getSkills().addXp(SkillType.MAGIC, xp);
					if (downEmoteId != -1)
	...
	public static boolean useTabTeleport(final Player player, final int itemId) {
		...
		if (useTeleTab(player, TABS[itemId - 8007]))
			player.getContent().get(ContentType.INVENTORY).deleteItem(itemId, 1);
		return true;
	}
