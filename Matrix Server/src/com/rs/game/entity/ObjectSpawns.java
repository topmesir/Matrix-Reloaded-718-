...
public final class ObjectSpawns {
	...
	private static final void packObjectSpawns() {
		...
		try {
			try(BufferedReader in = new BufferedReader(new FileReader("data/map/unpackedSpawnsList.txt"))) {
				String line;
	...
	public static final void loadObjectSpawns(int regionId) {
		...
		try {
			...
			while (buffer.hasRemaining()) {
				...
				World.spawnObject(new GameObject(objectId, type, rotation, x, y, plane), cliped);
			}
			channel.close();
			in.close();
